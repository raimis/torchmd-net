...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 15 15:07:34 2021
...
class BBADataset(InMemoryDataset):
    r""""""

    def __init__(self, root,  transform=None, pre_transform=None,
                 pre_filter=None):
        self.temperature = 350 # K
        super(BBADataset, self).__init__(root, transform, pre_transform, pre_filter)
    ...
    def download(self):
        ...
        url_trajectory = 'http://pub.htmd.org/bba_trajectories.tar.gz'
        url_forces = 'http://pub.htmd.org/bba_forces_nowater.tar.gz'
        url_coords = 'http://pub.htmd.org/bba_coords_nowater.tar.gz'
        url_inputs = 'http://pub.htmd.org/bba_generators.tar.gz'
    ...
    def raw_file_names(self):
        return ['bba_generators.tar.gz', 'bba_forces_nowater.tar.gz', 'bba_coords_nowater.tar.gz']
    ...
    def get_data_filenames(coord_dir, force_dir):
        tags = [os.path.basename(fn).replace('bba_coor_','').replace('.npy','') for fn in  os.listdir(coord_dir)]

        coord_fns = {}
        for tag in tags:
            fn = f'bba_coor_{tag}.npy'
            coord_fns[tag] = join(coord_dir, fn)
        forces_fns = {}
        for tag in tags:
            fn = f'bba_force_{tag}.npy'
        ...
        for i_traj, tag in enumerate(tqdm(coord_fns, desc='Load Dataset')):
            ...
            for i_frame in range(n_frames):
                ...
                data = Data(z=z, pos=pos, forces=force, idx=ii_frame,
                            name='bba', n_atoms=n_beads, traj_idx=i_traj)
