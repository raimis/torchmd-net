...
class Simulation(object):
    ...
    def __init__(self, model, initial_coordinates, embeddings, dt=5e-4,
                 ...
                 log_type='write', filename=None, batch_size=10, r_max=12):
    ...
    def calculate_potential_and_forces(self, x_old):
        ...
        for iframe in range(n_frames):
            ...
            data_list.append(AtomicData.from_points(x_old[iframe], self.r_max,\
                **{ 'atomic_numbers' : self.embeddings[iframe], 'idx' : iframe, 'n_atoms' : n_beads}).to(device=x_old.device))
...
class PTSimulation(Simulation):
    ...
    filename : string (default=None)
        ...
        for numpy outputs, '_coords_000.npy' or similar is added. For log
        outputs, '_log.txt' is added.
    coords_per_replica : bool (default=False) if true, give initial coordinates 
        for every replica-temperature combination, in this case len(betas=4) and
        initial coordinates of shape (40, n_atoms, 3) do not give 160 replicas, 
        but only 40 (10 per temperature).
    ...
    def __init__(self, model, initial_coordinates, embeddings, dt=5e-4,
        ...
        if coords_per_replica == False:
            ...
            self.embeddings = embeddings
            # set up betas for simulation
            assert len(initial_coordinates) % self.n_replicas == 0, "n_replicas has \
                to be a multiple of len(betas) if coords_per_replica=True!"
            self._n_indep = int(len(initial_coordinates)/len(betas))
            self._betas_x = np.repeat(self._betas, self._n_indep)
            self._betas_for_simulation = torch.tensor(self._betas_x[:, None, None],
                                                      dtype=torch.float32
                                                     ).to(self.device)
    ...
    #####################################################
    # from here on out, everything is as in Simulation()
    # exept self.beta -> self._betas_for_simulation in the integration steps
