...
class SchNet(Model):
    ...
    def __init__(self, embedding_size: int =100, hidden_channels: int =128,
        ...
        for _ in range(n_layers):
            layers_out.append(Linear(n_nodes, n_nodes // reduction_factor))
            layers_out.append(act_class())
            n_nodes = n_nodes // reduction_factor
        layers_out.append(Linear(n_nodes, 1))

        self.mlp = Sequential(*layers_out)
